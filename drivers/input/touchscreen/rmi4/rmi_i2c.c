				  ...
				  const struct i2c_device_id *id)
{
	...
	/* before rmi_set_page, must be control power for touch device */
	if (pdata->power_ctrl)
		pdata->power_ctrl(1);
	...
	printk(KERN_INFO "%s: registered rmi i2c driver at %#04x.\n",__func__,
			client->addr);
...
{
	printk(KERN_INFO "%s: %s added driver\n",
					__func__,rmi_i2c_driver.driver.name);
