...
static int read_f01_queries(struct reflash_data *data)
{
	...
	printk("Bootloader Firmware = 0x%x%x%x\n", 
		data->firmware_id[2], data->firmware_id[1], data->firmware_id[0]);
...
//	touch_firmware_version = dev_config_id;
	...
	dev_info(&data->rmi_dev->dev,"No need to do firmware/config update.\n");
	return NO_GO;
...
//	Pantech added for Touch monitor
	retval = read_f34_config_id(&data);

	if (retval != 0) 
		dev_err(&rmi_dev->dev, "Could not read firmware version info, code = %d\n", retval);

	touch_firmware_version = extract_config_id(data.config_id);
//End
