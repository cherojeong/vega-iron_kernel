...
#define CFG_F54_TXCOUNT 13//30
#define CFG_F54_RXCOUNT 23//46
#define PRINT_PDT_SCAN 0

unsigned char ImageBuffer[CFG_F54_TXCOUNT*CFG_F54_RXCOUNT*2];
signed short ImageArray[CFG_F54_TXCOUNT] [CFG_F54_RXCOUNT] ;
int ImageBuffer_int[CFG_F54_TXCOUNT*CFG_F54_RXCOUNT];
int ImageBuffer_int_temp[CFG_F54_TXCOUNT*CFG_F54_RXCOUNT];
signed short ImageBuffer_temp[CFG_F54_TXCOUNT*CFG_F54_RXCOUNT];
...
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
static struct rmi_function_container *fc_temp;
#if F54_WATCHDOG
...
extern u16 query_base_addr_f11;
extern u16 control_base_addr_f11;
extern u16 command_base_addr_f01;
extern u16 control_base_addr_f01;
extern u16 data_base_addr_f01;
...
int send_reference_data(unsigned long arg)
{
    ...
    if (copy_to_user(argp, ImageBuffer_int_temp, CFG_F54_TXCOUNT * CFG_F54_RXCOUNT * sizeof(int)))
		return 0;
	else
		return 1;
}
...
int send_reference_data_for_self(unsigned long arg)
{
    ...
    for(i = 0;i < CFG_F54_TXCOUNT * CFG_F54_RXCOUNT; i++) {
        if(ImageBuffer_int_temp[i] < 3200 || ImageBuffer_int_temp[i] > 5600) {
            // fail
            ImageBuffer_int[i] = 1;
            //retval = -1;
        }
    ...
    if (copy_to_user(argp, ImageBuffer_int, CFG_F54_TXCOUNT * CFG_F54_RXCOUNT * sizeof(int)))
		return 0;
	else
		return 1;
}
...
#if PRINT_PDT_SCAN
void rmi_print_function(void)
{
    printk("[rmi]fc -> fd.query_base_addr_f54 =  0x%x\n", f54_query_base_addr);
    printk("[rmi]fc -> fd.command_base_addr_f54 = 0x%x\n", f54_command_base_addr);
    printk("[rmi]fc -> fd.control_base_addr_f54 = 0x%x\n", f54_control_base_addr);   
    printk("[rmi]fc -> fd.data_base_addr_f54 = 0x%x\n", f54_data_base_addr);   
    printk("[rmi]fc -> fd.function_number_f54 = 0x%x\n", f54_function_number);
    printk("[rmi]f54_data_Lindex = 0x%x\n", f54_data_Lindex);
    printk("[rmi]f54_data_Hindex = 0x%x\n", f54_data_Hindex);
    printk("[rmi]f54_data_buffer = 0x%x\n", f54_data_buffer);
    printk("[rmi]data->f01_container->fd.control_base_addr_f01 = 0x%x\n", control_base_addr_f01);
    printk("[rmi]data->f01_container->fd.command_base_addr_f01 = 0x%x\n", command_base_addr_f01);
	printk("[rmi]query_base_addr_f11 = 0x%x\n", query_base_addr_f11);
    printk("[rmi]control_base_addr_f11 = 0x%x\n", control_base_addr_f11);
    printk("[rmi]f11_numTx = 0x%x\n", f11_numTx);
    printk("[rmi]f11_numRx = 0x%x\n", f11_numRx);
}
#endif

int rmi_scan_function(void)
{  
	if(fc_temp == NULL || fc_temp->rmi_dev == NULL)
		return false;

	//F54_SCAN
	f54_command_base_addr = fc_temp->fd.command_base_addr;
	f54_data_base_addr = fc_temp->fd.data_base_addr;	
	f54_data_Lindex = f54_data_base_addr + 1;
	f54_data_Hindex = f54_data_base_addr + 2;
	f54_data_buffer = f54_data_base_addr + 3;
	
	//F11_SCAN
	f11_numTx = query_base_addr_f11 + 2;
	f11_numRx = query_base_addr_f11 + 3;
    
	rmi_read_block(fc_temp->rmi_dev, f11_numRx, &MaxNumberRx, 1);
	rmi_read_block(fc_temp->rmi_dev, F55_PhysicalRx_Addr, &RxChannelUsed[0], MaxNumberRx);
	rmi_read_block(fc_temp->rmi_dev, f11_numTx, &MaxNumberTx, 1);
	rmi_read_block(fc_temp->rmi_dev, F55_PhysicalRx_Addr +1 , &TxChannelUsed[0], MaxNumberTx);
...
int rmi_raw_capacitance(void)
{
    ...
    length = CFG_F54_TXCOUNT * CFG_F54_RXCOUNT * 2;

	rmi_scan_function();
    
	command = 0x00;
	result = rmi_write_block(fc_temp->rmi_dev, F54_CBCSettings, &command, 1);
	
	command = 0x04;
	result = rmi_write_block(fc_temp->rmi_dev, control_base_addr_f01, &command, 1);

	command = 0x03;
	result = rmi_write_block(fc_temp->rmi_dev, f54_data_base_addr, &command, 1);

	command = 0x01;
	result = rmi_write_block(fc_temp->rmi_dev, NoiseMitigation, &command, 1);

	command = 0x06; //Force update & Force cal
	result = rmi_write_block(fc_temp->rmi_dev, f54_command_base_addr, &command, 1);

   	for (i = 0; i < 20; i++) //for test 
    {
        mdelay(10);
		rmi_read_block(fc_temp->rmi_dev, f54_command_base_addr, &command, 1);
    }
	
	command = 0x08;
	result = rmi_write_block(fc_temp->rmi_dev, command_base_addr_f01 + 1, &command, 1);
	
	command = 0x00;
	result = rmi_write_block(fc_temp->rmi_dev, f54_data_Lindex, &command, 1);
	result = rmi_write_block(fc_temp->rmi_dev, f54_data_Hindex, &command, 1);
	...
	rmi_read_block(fc_temp->rmi_dev, f54_data_buffer, &ImageBuffer[0], length);
	...
	for (i = 0; i < CFG_F54_TXCOUNT; i++)
	{
		...
		for (j = 0; j < CFG_F54_RXCOUNT; j++)
			{       
                ...
                if (temp > - 10 && temp < 6000)
                    retval++;
				//printk("%4d\t",  temp); //It's for getting log for limit set
				k = k + 2;
			}	   
    ...
    command = 0x01; //reset
    rmi_write_block(fc_temp->rmi_dev, command_base_addr_f01, &command, 1);
    mdelay(200);
    rmi_read_block(fc_temp->rmi_dev, data_base_addr_f01, &command, 1);

	if (retval == CFG_F54_TXCOUNT * CFG_F54_RXCOUNT)
		return 0; //SUCESS!!
	else
		return 1; //FAIL!!
}
...
int rmi_baseline(int limit)
{
	...
	command = 0x04; //no sleep

	result = rmi_write_block(fc_temp->rmi_dev, control_base_addr_f01, &command, 1);

	if(result < 0)
		printk("[Touch][%s] write fail line: %d\n",__FUNCTION__,__LINE__);
	...
	for (i = 0, k=0; i < CFG_F54_TXCOUNT; i++)
	{
		for (j = 0; j < CFG_F54_RXCOUNT; j++)
		{
	...
	for (i = 0; i < CFG_F54_RXCOUNT; i++)
	{
		...
		{
              	ImageBuffer_temp[13*i+j] = (short)(ImageBuffer[k] | (ImageBuffer[k+1] << 8));
              	ImageBuffer_int_temp[13*i+j] = (int)(ImageBuffer_temp[13*i+j]);
 //             	printk("%2d\t",ImageBuffer_int_temp[13*i+j]);
              	if (ImageBuffer_int_temp[13*i+j] > 3200 && ImageBuffer_int_temp[13*i+j] < 5600)
                    	retval++;
              	k += 2;
		}
    	 //printk("\n");
	...
	if (retval >= /*CFG_F54_TXCOUNT * CFG_F54_RXCOUNT*/ limit)
		return true; //SUCESS!!
	else
		return false; //FAIL!!
...
{  
	...
	// Print Capacitance Imgae for getting log for Excel format
	for (i = 0, k=0; i < CFG_F54_RXCOUNT; i++)
	{
		for (j = 0; j < CFG_F54_TXCOUNT; j++)
		{
			ImageBuffer_temp[13*i+j] = (short)(ImageBuffer[k] | (ImageBuffer[k+1] << 8));
...
static int __init rmi_f54_module_init(void)
{
	...
	printk(KERN_INFO "%s: register successful - fn%x\n",__func__,
